...
<title>UAV-VQA 语义通信系统：设计与实验结果 v4（2026-07-05）</title>
<style>
...
<h1>UAV-VQA 语义通信系统：设计与实验结果 <span style="color:var(--acc)">v4</span></h1>
<p class="sub">2026-07-05 · 数据来自 160 服务器全量重算（与原 182 结果逐点复现校验通过）·
仓库 <code>qiankun-phd/uav-vqa-semantic-rl</code> 分支 <code>codex/lut-semantic-utility-upgrade</code>（对比实验）/ <code>main</code> + <code>feat/bubbles-scenario</code>（v19 资源分配 RL）·
<b>v3 新增</b>：跨数据集 DroneVehicle 验证（§12）、语义通信进 BUBBLES 飞行安全环（§13）、新颖性与期刊定位（§14）、
§6 跨三 VLM 家族升级 ·
<b>v4 新增</b>：v19 算法双模型审查、P0/P1 修复、BUBBLES 主线三轮 A/B 消融与"结构承重 + 对偶定价"定论（§15）</p>
...
<a href="#xdata">12 跨数据集</a> <a href="#safety">13 飞行安全环</a> <a href="#novelty">14 新颖性与定位</a>
<a href="#v19audit">15 v19 审查与训练级验证</a></nav>
...
<div class="card"><b>−0.168</b><span>慢速移动头因果降低冲突率（BUBBLES 主线 B1 消融，v2/v3 两轮稳定）——约束满足由结构承担的最强单项证据（§15）</span></div>
</div>
...
{RL}
<div class="note"><b>v4 更新</b>：本章描述的拉格朗日对偶机制经三轮 A/B 消融证实为<b>定价而非控制</b>——
约束满足实际由慢速移动头 / 安全投影等结构件承担，λ 收敛为稳定影子价格，详见 §15。</div>
...
<div class="finding">⑫ <b>对偶通道不承重——"结构承重 + 对偶定价"</b>（§15）：三轮递进消融（关 λ_conflict 通道 / 单通道化后复测 / λ 热启动）下 B2 与 A2 行为<b>逐位相同</b>，冲突约束满足实际由慢速移动头（−0.168）与安全投影（−0.21）等结构件承担；λ 收敛为稳定影子价格（度量约束紧迫度）而非控制手段——一个干净的 <b>primal 钝化</b>案例。</div>
...
<tr><td><b>v4：v19 审查与 BUBBLES A/B（§15）</b></td><td><code>feat/bubbles-scenario</code> 分支 <b>8 个提交</b>：<code>74e4991</code> 场景档、
<code>541c39c</code>/<code>428aec7</code>/<code>2cd5212</code>/<code>3999ad6</code> P0+P1 修复与单测门、<code>b176b52</code>/<code>1b9902d</code> A2warm λ 热启动消融 等</td></tr>
<tr><td>双模型审查合并结论</td><td><code>docs_spec/V19_Design_Review_2026-07.md</code>（Opus + Fable 独立只读审查，零矛盾）</td></tr>
<tr><td>A/B 消融汇总（160）</td><td><code>outputs/rl/ab_bubbles_v3/ab_summary.md</code>（BUBBLES 主线 v3 双工况全表 + λ 轨迹）</td></tr>
</table>
...
<h2 id="v19audit">15 v19 算法审查与训练级验证（BUBBLES 主线）</h2>
<p>§2 描述了 v19 架构的"设计意图"；本章回答"它实际在做什么"。工作分四步：
<b>双模型独立审查</b>（15.1）→ <b>P0/P1 修复</b>（15.2）→ <b>BUBBLES 主线 A/B</b>（15.3）→
<b>三轮对偶消融</b>（15.4），最终收敛到"结构承重 + 对偶定价"的叙事定论（15.5）。
合并审查结论全文见 <code>docs_spec/V19_Design_Review_2026-07.md</code>。</p>

<h3>15.1 双模型独立审查（Opus + Fable，零矛盾）</h3>
<p>两个代理对 <code>main @ ad31e9c</code> + BUBBLES worktree + 160 旧训练产物做<b>独立只读审查</b>，
交叉验证后结论<b>零矛盾</b>、关键发现互证。判定总表：</p>
<table>
...
<tr><td>慢头 credit assignment</td><td class="bad">真 P0</td><td>慢速移动头 log_prob <b>每步计入（×K=3）</b>；且 mobility mask 采样端 masked / 更新端 unmasked → ratio≠1 虚假触发 clip，熵也全步计入</td></tr>
<tr><td>两时标 mobility 代价双计</td><td class="bad">真 P0</td><td>semantic_utility 奖励内已含 mobility_cost，两时标路径又叠加 <code>_mobility_reward_adjustment</code>——旧"two-timescale vs monolithic"消融差异部分来自奖励定义不同，<b>该消融表失真，须修后重跑</b></td></tr>
<tr><td>λ 棘轮</td><td class="bad">真 P1</td><td>limit=0 且 cost≥0 → λ 单调不减、爬升顶格；旧 trace 实证 λ_quality <b>0→5.41 全程单调</b>、违约率 0.9 始终不降——λ 是障碍项不是影子价格</td></tr>
<tr><td>冲突多重罚</td><td class="bad">真 P1</td><td>BUBBLES 冲突同时置 airspace+utm 双 flag → shaped <b>−3.5 重罚</b>，再叠 λ 项 + Lyapunov 水位罚 + 隐性第 4 通道（success 翻负）；后期冲突步净罚 −30~−50 vs 正项上限 +23，cache 塌缩是理性解</td></tr>
<tr><td>BC 贴示范锁死</td><td class="bad">真 P1</td><td><code>bc_aux_weight=0.28</code> <b>永不衰减</b>（只有 0.65 先验衰减）；实证 low_snr 下 proposed≈教师（0.955 vs 0.950）</td></tr>
<tr><td>CPA 用 nearest UAV</td><td class="bad">真 P1-env</td><td>CPA 运动学用 <b>nearest 而非 assigned UAV</b>（multi_uav_env.py:1918）→ 冲突对策略基本外生，唯一规避手段是选 cache——冲突可控性的环境侧根因</td></tr>
<tr><td>advantage 归一化中和 λ</td><td class="bad">真 P1</td><td>per-episode 归一化把 episode 内近常数的 λ 罚减均值消掉 → 对偶机制一阶效应被架空</td></tr>
<tr><td>欠训练 + 高方差</td><td class="bad">真 P2</td><td>120ep 回报仍在涨、末期 std&gt;|mean|、容量非瓶颈；rollout 采集缺 no_grad</td></tr>
</table>

<h3>15.2 P0 + P1 修复（feat/bubbles-scenario）</h3>
<p>修复落在 <code>feat/bubbles-scenario</code> 分支提交 <code>541c39c</code> / <code>428aec7</code> / <code>2cd5212</code> / <code>3999ad6</code>：</p>
<table>
...
<tr><td>单测门</td><td><b>epoch-0 ratio≡1</b>：采集后立即重算 log_prob 断言与 old_log_probs 逐步相等；<b>133 项测试全过</b></td></tr>
<tr><td>λ 通道（P1）</td><td>λ 更新加<b>泄漏项</b> + 分通道 λ 上限（棘轮解除，λ 可降）</td></tr>
...
<tr><td>归一化 / 效率</td><td>advantage 改 <b>scale-only 归一化</b>（保均值、不消 λ 罚）；rollout 采集包 no_grad</td></tr>
</table>

<h3>15.3 BUBBLES 主线 A/B（v3，单场景双工况）</h3>
<p>Arms：<b>A2</b>（bubbles 全量，修复后）/ <b>B1</b>（无慢速移动头）/ <b>B2</b>（关 λ_conflict 通道）/
<b>A1</b>（legacy 档，奖励尺度不同、只作参照）/ <b>Cgreedy</b>、<b>Ccache</b>（非学习基线）。
数据：<code>outputs/rl/ab_bubbles_v3/ab_summary.md</code>（160）。</p>
...
<tr><td>B2 关 λ_conflict</td><td class="num">0.5628</td><td class="num">0.3027</td><td class="num">0.3907</td><td><b>与 A2 逐位相同</b>——对偶通道不承重（见 15.4）</td></tr>
...
<tr><td>A1 legacy（尺度不可比）</td><td class="num">0.5332</td><td class="num">0.1147</td><td class="num">—</td><td>奖励尺度不同，仅作参照，不进结论</td></tr>
<tr><td>Cgreedy</td><td class="num">0.3859</td><td class="num bad">0.6540</td><td class="num">—</td><td>贪心全发：冲突失控</td></tr>
<tr><td>Ccache</td><td class="num bad">0.3127</td><td class="num best">0.0000</td><td class="num">—</td><td>全缓存：零冲突但精度塌缩</td></tr>
</table>
<p><b>nominal 工况</b>：</p>
<table>
...
<tr><td>Cgreedy</td><td class="num">0.6241</td><td class="num">0.8960</td><td class="num best">0.3680</td><td class="num">—</td></tr>
<tr><td>Ccache</td><td class="num bad">0.2655</td><td class="num bad">0.2970</td><td class="num">—</td><td class="num">—</td></tr>
</table>
<p><b>λ_conflict 收敛区间</b>：A2 <b>3.7–4.7</b>（末段均值 ≈4.31）；B1 <b>6.6–7.1</b>（结构件缺位 → 影子价格更高，方向正确）；
A1 <b>0.8–1.1</b>。修复后 λ 不再棘轮顶格，随约束紧迫度分层——<b>作为度量是好的</b>；作为控制是否有效见 15.4。</p>

<h3>15.4 三轮对偶消融与 λ 热启动否证</h3>
<ol style="font-size:14px">
<li><b>第一轮（v2）</b>：关掉 λ_conflict 通道的 B2 与全量 A2 行为完全相同——首次发现对偶通道可能不承重；
当时怀疑是多通道罚冗余掩盖。</li>
<li><b>第二轮（v3，罚通道单通道化后）</b>：去重后复测，<b>B2 仍与 A2 逐位相同</b>（冲突率 0.3027 逐位同、全指标逐位同）——
排除"冗余通道顶替"解释。剩余假设：λ 从 0 起步"迟到"，等它长起来时策略已被结构件带进无冲突区。</li>
<li><b>第三轮（A2warm，提交 <code>b176b52</code>/<code>1b9902d</code>）</b>：λ_conflict=4.0 <b>热启动、全程在场</b>——
结果<b>仍与 A2 逐位相同</b>。"迟到"假设被否证。</li>
</ol>
<div class="finding"><b>深挖（为何梯度在流、行为却不变）</b>：A2warm 与 A2 的网络权重确有差异
（max|Δw| = 0.35，λ 罚的梯度确实流过并改了参数），但策略行为<b>重新收敛到同一解</b>；
且 λ 三种起法（0 起步 / 单通道 / 4.0 热启动）均收敛到 <b>3.7–4.7 同一均衡带</b>——
对偶变量把"价格"算对了，但 primal 侧被安全投影、mask、资源 floor 等硬结构钉死，价格信号改不动行为：
<b>价格对、控制无效——primal 钝化</b>（primal saturation）。</div>

<h3>15.5 定论："结构承重 + 对偶定价"</h3>
<p>三轮消融给出干净的机制分解——<b>约束满足由结构承担，λ 只负责定价</b>：</p>
<table>
<tr><th>承重件</th><th class="num">因果贡献</th><th>证据</th></tr>
<tr><td><b>慢速移动头</b>（结构）</td><td class="num best">冲突率 −0.168</td><td>B1 消融，v2/v3 两轮稳定复现（0.4707 → 0.3027）</td></tr>
<tr><td><b>安全投影层</b>（结构）</td><td class="num best">semSucc −0.21</td><td>旧组件消融最大单项（去掉后向 cache 塌缩）</td></tr>
<tr><td><b>隐式通道</b>（奖励结构）</td><td class="num">冲突→任务失败丢 +6 正项</td><td>冲突步 success 翻负，本身就是强规避信号——无需 λ 也在罚</td></tr>
<tr><td>λ_conflict（对偶）</td><td class="num">行为贡献 ≈ 0</td><td>三轮消融逐位相同；但收敛为<b>稳定影子价格</b>（A2 3.7–4.7 / B1 6.6–7.1 分层正确），可作约束紧迫度度量</td></tr>
</table>
...
用作约束紧迫度的在线度量"</b>。三轮递进消融（发现 → 排除冗余解释 → 否证迟到假设 → 权重-行为深挖）本身构成论文二的核心证据链——
一个方法论上干净的 primal 钝化案例。</div>
<p><b>Future work（救活对偶控制的两条路，标注不实施）</b>：
（1）<b>减弱安全投影</b>，把部分约束满足责任让渡给 λ，给价格信号留出改变行为的裕度；
（2）<b>credit assignment 对准慢头决策步</b>——把冲突代价精确记账到产生它的机动决策上（而非按 slot 摊销），
让 λ 罚的梯度作用在真正可控的自由度上。</p>

</body></html>"""

out = "/Users/zhangqiankun/Documents/mpu/HPPO-VQA/docs_spec/UAV_VQA_SemCom_System_and_Results_v4.html"
