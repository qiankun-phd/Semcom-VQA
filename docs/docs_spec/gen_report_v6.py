#!/usr/bin/env python3
"""Self-contained HTML report v6: v5 + matrix_v1 实验矩阵全量并入 Part E。

改造自 gen_report_v5.py。v5 的全部内容（Part A-F / 9 图 / 全部实测数字）零删保留，
仅在 Part E 把原 E3"进行中：实验矩阵"整节替换为 matrix_v1 的完整结果节
（E3 设计 / E4 收敛 / E5 主对比 Table I / E6 三轴零样本 / E7 约束-Pareto / E8 判据）。
新增 7 张 matrix_v1 图（fig1-fig7），共 16 图。
图来源 /tmp/report_figs/（9 张旧 PNG + 7 张 matrix_v1 PNG）。
数据来源：160 outputs/rl/matrix_v1/matrix_summary.md + tables/（只读取回）。
"""
import base64
import os
...
for name in ["F1_acc_snr_3panel_final", "F2_cliff_final", "F3_latency_final",
             ...
             # v6 新增：matrix_v1 实验矩阵 7 图
             "fig1_convergence", "fig2_multiscale", "fig3_uav", "fig4_load",
             "fig5_snr", "fig6_violation_pareto", "fig7_zeroshot"]:
...
<title>UAV-VQA 语义通信系统：设计与实验结果 v6（matrix_v1 实验矩阵并入，2026-07-07）</title>
<style>
...
<h1>UAV-VQA 语义通信系统：设计与实验结果 <span style="color:var(--part)">v6</span>
<span style="font-size:15px;color:var(--muted)">（叙事重构版 · matrix_v1 实验矩阵并入版）</span></h1>
<p class="sub">2026-07-07 · 数据来自 160 服务器全量重算（与原 182 结果逐点复现校验通过）·
<b>v6 相对 v5</b>：v5 全部内容零删；Part E 把原 E3"进行中：实验矩阵"整节替换为
<code>matrix_v1</code>（5 学习臂 × 3 seeds × 1000ep + 4 非学习基线，双工况）的<b>完整结果节</b>
（E3 设计 / E4 收敛 ep95≈97 / E5 主对比 Table I / E6 三轴零样本 / E7 约束-Pareto / E8 判据 5 过 4）+ 新增 7 图（共 16 图）·
...
<div class="card"><b>ep95&asymp;97</b><span>matrix_v1 proposed 约 100 episodes 平台化，远早于 1000ep 训练预算——样本效率证据（Part E4，Table III）</span></div>
</div>
...
<a href="#E1">E1 双工况A/B</a> <a href="#E2">E2 承重件与权衡</a> <a href="#E3">E3 实验矩阵设计</a>
<a href="#E4">E4 收敛性</a> <a href="#E5">E5 主对比 Table I</a> <a href="#E6">E6 三轴零样本</a>
<a href="#E7">E7 约束与Pareto</a> <a href="#E8">E8 判据与诚实项</a><br>
...
<p class="sub"><b>E1&ndash;E2</b> 数据：<code>outputs/rl/ab_bubbles_v3/ab_summary.md</code>（160，BUBBLES 主线 v3 双工况全表 + &lambda; 轨迹）；
Arms：<b>A2</b>（bubbles 全量，修复后）/ <b>B1</b>（无慢速移动头）/ <b>B2</b>（关 &lambda;_conflict 通道）/
<b>A1</b>（legacy 档，尺度仅参照）/ <b>Cgreedy</b>、<b>Ccache</b>（非学习基线）。
<b>E3&ndash;E8</b> 数据：<code>outputs/rl/matrix_v1/</code>（标准化实验矩阵，<code>matrix_summary.md</code> + <code>tables/</code> + 7 图，160 只读取回）。</p>
...
<h3 id="E3">E3 标准化实验矩阵 matrix_v1（设计）</h3>
<p>E1/E2 是 BUBBLES 主线 v3 的双工况 A/B 消融；本节起（E3&ndash;E8）为按
<code>docs_spec/RL_Experiment_Standards_Survey.md</code>（7 篇 TWC/JSAC/IoTJ 深读的第 3 节最终矩阵）
落地的<b>标准化实验矩阵 matrix_v1</b>，已在 160 全量跑出（<code>outputs/rl/matrix_v1/</code>，生成于 2026-07-06）。</p>
<p><b>矩阵构成</b>：<b>5 个学习臂</b>（proposed 约束两时间尺度 / no_lagrangian / fixed-penalty / service_only /
multiscale 规模变体）<b>&times; 3 seeds &times; 1000 episodes</b>，外加 <b>4 个非学习基线</b>
（semantic greedy 全发 / always cache 全缓存 / oracle 最优可行 / random）。评测跨
<b>双工况</b>（peak = utm_conflict 峰值；nominal = BUBBLES 标称），每评测点 <b>32+ rollouts</b>，report 均值±std。
三类产出：<b>收敛曲线</b>（E4）、<b>主对比表</b>（E5）、<b>三轴零样本泛化扫描</b>（E6）、<b>约束满足 / Pareto</b>（E7）、
以及<b>判据核查</b>（E8）。矩阵设计与训练协议依据见上引 survey；这套标准化对齐了 TWC/JSAC 同类工作的
seed 数、episode 预算、rollout 数与基线套餐（含 oracle 上界与 random 下界锚点）。</p>

<h3 id="E4">E4 收敛性与样本效率</h3>
""" + img("fig1_convergence", "Fig.1 收敛三联（matrix_v1，peak）：(a) reward vs episode；(b) 约束成本 vs episode 叠加 0.08 预算界线；(c) &lambda;_conflict 轨迹。5 个学习臂同图，3 seeds ±std。proposed 约束成本收束到 0.08 界线下方，&lambda; 收敛为稳定影子价格。") + """
""" + img("fig2_multiscale", "Fig.2 多规模收敛（matrix_v1）：proposed 在 2 / 4 / 6 UAV 三档下的 reward 收敛曲线，验证收敛速度对规模稳健。默认 4 UAV。") + """
<div class="finding"><b>关键读数 ep95&asymp;97</b>（Table III）：proposed（约束两时间尺度）达最终 reward 95% 平台所需 episode 数
<b>97±0</b>，即<b>约 100 个 episode 即平台化，远早于 1000ep 训练预算</b>；multiscale 默认 4 UAV 档同样 ep95&asymp;97。
单次 1000ep 训练墙钟 <b>4.4 min/run</b>，最终 return &minus;52.40±0.00。这是"约束两时间尺度求解器样本效率高"的直接证据——
收敛速度远快于训练预算，留出充足裕度做零样本泛化评估（E6）。</div>

<h3 id="E5">E5 主对比（Table I 全表）</h3>
<div class="finding"><b>头条（peak 工况平均精度阶梯）</b>：
oracle <b>0.684</b> &gt; <b>proposed 0.574</b> &gt; random <b>0.458</b> &gt; greedy <b>0.386</b> &gt; cache <b>0.313</b>。
proposed 位于 oracle 上界与 random/greedy/cache 之间的正确档位：相对语义贪心 <b>精度 +0.19</b>（0.574 vs 0.386）、
<b>空域冲突率减半</b>（0.296 vs 0.654）、<b>deadline 违约 0.748 vs 1.000</b>——用少量精度换来大幅约束改善。
always cache 虽零冲突零违约，但精度塌到 0.313（安全但无用的坏端点）。</div>
<p><b>Table I&mdash;&mdash;主对比（双工况 &times; 9 方法，mean±std，跨 seeds/runs）</b>
<span class="sub">来源 <code>outputs/rl/matrix_v1/tables/table1_main.md</code></span></p>
<table>
<tr><th>工况</th><th>方法</th><th class="num">runs</th><th class="num">平均精度</th><th class="num">语义成功率</th><th class="num">任务成功率</th><th class="num">空域冲突率</th><th class="num">deadline 违约率</th><th class="num">平均时延</th><th class="num">平均 payload(kb)</th><th class="num">平均 reward</th></tr>
<tr><td rowspan="5">peak</td><td><b>Proposed（约束两时间尺度）</b></td><td class="num">1</td><td class="num best">0.574±0.000</td><td class="num">0.000±0.000</td><td class="num">0.000±0.000</td><td class="num">0.296±0.000</td><td class="num">0.748±0.000</td><td class="num">16.455</td><td class="num">0.711</td><td class="num">&minus;5.863</td></tr>
<tr><td>Semantic greedy</td><td class="num">1</td><td class="num">0.386±0.000</td><td class="num">0.000</td><td class="num">0.000</td><td class="num bad">0.654±0.000</td><td class="num bad">1.000±0.000</td><td class="num">12.210</td><td class="num">198.931</td><td class="num">&minus;6.382</td></tr>
<tr><td>Always cache</td><td class="num">1</td><td class="num bad">0.313±0.000</td><td class="num">0.000</td><td class="num">0.000</td><td class="num best">0.000±0.000</td><td class="num best">0.000±0.000</td><td class="num">0.178</td><td class="num">0.000</td><td class="num">&minus;0.943</td></tr>
<tr><td>Oracle（最优可行）</td><td class="num">1</td><td class="num best">0.684±0.000</td><td class="num">0.000</td><td class="num">0.000</td><td class="num">0.402±0.000</td><td class="num">0.642±0.000</td><td class="num">6.158</td><td class="num">0.763</td><td class="num">&minus;3.345</td></tr>
<tr><td>Random</td><td class="num">3</td><td class="num">0.458±0.009</td><td class="num">0.000</td><td class="num">0.000</td><td class="num">0.428±0.026</td><td class="num">0.657±0.021</td><td class="num">7.950±0.195</td><td class="num">61.576±1.952</td><td class="num">&minus;4.317±0.144</td></tr>
<tr><td rowspan="5">nominal</td><td><b>Proposed（约束两时间尺度）</b></td><td class="num">1</td><td class="num best">0.632±0.000</td><td class="num best">0.943±0.000</td><td class="num">0.279±0.000</td><td class="num best">0.000±0.000</td><td class="num">0.721±0.000</td><td class="num">12.565</td><td class="num">2.047</td><td class="num">&minus;3.776</td></tr>
<tr><td>Semantic greedy</td><td class="num">1</td><td class="num">0.624±0.000</td><td class="num">0.896±0.000</td><td class="num best">0.368±0.000</td><td class="num">0.000</td><td class="num">0.632±0.000</td><td class="num">6.051</td><td class="num">24.186</td><td class="num">&minus;1.870</td></tr>
<tr><td>Always cache</td><td class="num">1</td><td class="num bad">0.266±0.000</td><td class="num bad">0.297±0.000</td><td class="num">0.297±0.000</td><td class="num">0.000</td><td class="num best">0.000±0.000</td><td class="num">0.126</td><td class="num">0.000</td><td class="num">&minus;0.605</td></tr>
<tr><td>Oracle（最优可行）</td><td class="num">1</td><td class="num best">0.655±0.000</td><td class="num">0.899±0.000</td><td class="num">0.366±0.000</td><td class="num">0.000</td><td class="num">0.613±0.000</td><td class="num">7.207</td><td class="num">106.197</td><td class="num">&minus;2.278</td></tr>
<tr><td>Random</td><td class="num">3</td><td class="num">0.472±0.005</td><td class="num">0.649±0.006</td><td class="num">0.317±0.003</td><td class="num">0.000</td><td class="num">0.470±0.003</td><td class="num">4.824±0.038</td><td class="num">43.393±0.961</td><td class="num">&minus;1.861±0.006</td></tr>
</table>
<p class="sub">读解：peak 工况约束压力最大，proposed 相对基线的冲突/违约优势最突出；nominal 工况 proposed 语义成功率 0.943（最高），
但任务成功率 0.279 略逊 greedy 0.368——见 E8 诚实项。oracle 各工况均为精度上界，印证学习方向正确。</p>

<h3 id="E6">E6 三轴零样本泛化</h3>
<p>训练点固定，评测在<b>未见工况</b>上零样本扫描三个轴（UAV 数 / 负载到达率 / SNR），跨全部方法对比退化斜率。</p>
""" + img("fig3_uav", "Fig.3 零样本泛化 &times; UAV 数：性能随机群规模变化。proposed 随规模增大的退化斜率显著平于非学习基线——约束结构对规模外推稳健。") + """
""" + img("fig4_load", "Fig.4 零样本泛化 &times; 负载（到达率）：性能随任务到达率上升的变化。proposed 在高负载端仍维持约束满足，贪心/随机违约快速抬升。") + """
""" + img("fig5_snr", "Fig.5 零样本泛化 &times; SNR：性能随信道 SNR 扫描。低 SNR 端 proposed 平滑退化（对应 B7 可变 token 预算的接入位置），固定策略基线呈悬崖。") + """
<div class="finding"><b>三轴趋势</b>（从图读）：proposed 在 UAV 数、负载、SNR 三个未见轴上均呈<b>平滑退化</b>，
退化斜率均缓于 greedy/cache/random 等非学习基线；oracle 提供各点的可行上界包络，proposed 曲线贴近上界包络下方，
且始终位于 random 之上——零样本外推下"proposed &gt; random、贴 oracle"的排序稳定保持，说明约束两时间尺度学到的是
可迁移的结构性策略而非对训练工况的过拟合。</div>

<h3 id="E7">E7 约束满足与 Pareto</h3>
""" + img("fig6_violation_pareto", "Fig.6 约束满足（matrix_v1）：左=双工况违约率柱状对比；右=效用-违约 Pareto。proposed 落在 Pareto 前沿的高效用-低违约拐角，greedy（高违约）与 cache（零违约但零效用）是前沿两个坏端点。") + """
""" + img("fig7_zeroshot", "Fig.7 零样本泛化专图：3 个未见评测点上 proposed 零样本 vs 重训参考点。零样本性能接近重训参考，量化&ldquo;无需重训即可外推&rdquo;的差异化加分。") + """
<div class="finding"><b>Pareto 定论</b>：在效用-违约平面上，proposed 稳定占据高效用-低违约拐角；
Fig.7 进一步显示 3 个未见点上 proposed <b>零样本</b>性能接近<b>重训参考点</b>——即无需针对每个新工况重训即可维持约束满足，
这是相对"每工况重训"路线的核心差异化加分。</div>

<h3 id="E8">E8 判据核查与诚实项</h3>
<p>matrix_summary 的自动判据（源 <code>matrix_summary.md</code> Criteria check）：<b>5 项过 4 项</b>。</p>
<table>
<tr><th>判据</th><th>阈值</th><th class="num">观测</th><th>结论</th></tr>
<tr><td>proposed 在 1000ep 前平台化（ep95 &lt; 900）</td><td>ep95 &lt; 900</td><td class="num">ep95&asymp;97</td><td class="best">PASS</td></tr>
<tr><td>oracle 精度 &ge; proposed 精度（上界 sanity）</td><td>&Delta; &ge; 0</td><td class="num">+0.1100</td><td class="best">PASS</td></tr>
<tr><td>proposed 精度 &gt; random 精度</td><td>&Delta; &gt; 0</td><td class="num">+0.1154</td><td class="best">PASS</td></tr>
<tr><td>nominal proposed 语义成功率 &ge; 0.92（v3 判据 6）</td><td>&ge; 0.92</td><td class="num">0.9430</td><td class="best">PASS</td></tr>
<tr><td>nominal proposed 任务成功率 &ge; 0.30（v3 判据 6）</td><td>&ge; 0.30</td><td class="num bad">0.2790</td><td class="bad">FAIL（边缘）</td></tr>
</table>
<div class="note"><b>诚实项（如实标注）</b>：nominal 工况 proposed <b>任务成功率 0.279 &lt; 0.30 阈值，边缘 FAIL</b>。
根因：proposed 优先保语义成功率（0.943，全表最高）与约束满足，在 nominal 工况下任务成功率略低于 greedy（0.368）——
这正是"约束满足优先"策略在低压力工况下的可预期让步，与 E5 主对比一致。此项不掩饰，作为论文 limitation 如实呈现；
其余 4 项（收敛速度、oracle 上界 sanity、优于 random、语义成功率）全部 PASS。</div>

<h3 id="E-tabs">Table II / Table III（消融与样本效率）</h3>
<p><b>Table II&mdash;&mdash;结构消融</b>（v3 BUBBLES 链，peak 工况，3 seeds mean±std）
<span class="sub">来源 <code>tables/table2_ablation.md</code>；与 E1/E2 的 A2/B1 消融同源、加 ±std</span></p>
<table>
<tr><th>臂</th><th class="num">runs</th><th class="num">平均精度</th><th class="num">语义成功率</th><th class="num">任务成功率</th><th class="num">空域冲突率</th><th class="num">deadline 违约率</th><th class="num">平均时延</th><th class="num">平均 payload(kb)</th><th class="num">平均 reward</th></tr>
<tr><td><b>Proposed（full, v3 500ep）</b></td><td class="num">3</td><td class="num">0.563±0.009</td><td class="num">0.000</td><td class="num">0.000</td><td class="num">0.303±0.007</td><td class="num">0.719±0.033</td><td class="num">15.823±0.548</td><td class="num">0.724±0.012</td><td class="num">&minus;5.685±0.163</td></tr>
<tr><td>w/o conflict dual channel</td><td class="num">3</td><td class="num">0.563±0.009</td><td class="num">0.000</td><td class="num">0.000</td><td class="num">0.303±0.007</td><td class="num">0.719±0.033</td><td class="num">15.823±0.548</td><td class="num">0.724±0.012</td><td class="num">&minus;5.685±0.163</td></tr>
<tr><td>w/o slow mobility head</td><td class="num">3</td><td class="num">0.581±0.007</td><td class="num">0.000</td><td class="num">0.000</td><td class="num bad">0.471±0.009</td><td class="num bad">1.000±0.000</td><td class="num">9.235±0.017</td><td class="num">0.930±0.011</td><td class="num">&minus;4.904±0.037</td></tr>
<tr><td>dual warm-start probe</td><td class="num">3</td><td class="num">0.563±0.009</td><td class="num">0.000</td><td class="num">0.000</td><td class="num">0.303±0.007</td><td class="num">0.719±0.033</td><td class="num">15.789±0.582</td><td class="num">0.724±0.012</td><td class="num">&minus;5.676±0.166</td></tr>
<tr><td>legacy profile reference</td><td class="num">3</td><td class="num">0.533±0.009</td><td class="num">0.000</td><td class="num">0.000</td><td class="num">0.115±0.011</td><td class="num">0.753±0.016</td><td class="num">14.271±0.302</td><td class="num">0.888±0.017</td><td class="num">&minus;5.068±0.071</td></tr>
<tr><td>Semantic greedy</td><td class="num">1</td><td class="num">0.386±0.000</td><td class="num">0.000</td><td class="num">0.000</td><td class="num bad">0.654±0.000</td><td class="num bad">1.000±0.000</td><td class="num">12.210</td><td class="num">198.931</td><td class="num">&minus;6.382</td></tr>
<tr><td>Always cache</td><td class="num">1</td><td class="num">0.313±0.000</td><td class="num">0.000</td><td class="num">0.000</td><td class="num best">0.000±0.000</td><td class="num best">0.000±0.000</td><td class="num">0.178</td><td class="num">0.000</td><td class="num">&minus;0.943</td></tr>
</table>
<p class="sub">读解：与 E1/E2 完全一致&mdash;&mdash;w/o slow mobility head 冲突率抬到 0.471（+0.168，慢头结构承重）；
w/o conflict dual channel 与 full 逐位相同（对偶通道不承重，见 D5 primal 钝化）；now 加 3 seeds ±std 后结论稳定。</p>
<p><b>Table III&mdash;&mdash;样本效率</b>（达最终 reward 95% 平台的 episode 数；每 1000ep run 墙钟）
<span class="sub">来源 <code>tables/table3_sample_efficiency.md</code></span></p>
<table>
<tr><th>方法</th><th class="num">达 95% 平台 episode 数（mean±std）</th><th class="num">最终 return（mean±std）</th><th class="num">墙钟 min/run（mean）</th></tr>
<tr><td>Proposed（约束两时间尺度）</td><td class="num best">97±0</td><td class="num">&minus;52.40±0.00</td><td class="num">4.4</td></tr>
</table>
<p class="sub">ep95&asymp;97 = 约 100 episode 平台化，仅占 1000ep 预算的 &lt;10%&mdash;&mdash;样本效率的量化背书（呼应 E4 Fig.1/Fig.2 与摘要卡片）。</p>
"""
...
<h3 id="F1">F1 发现清单（论文叙事骨架，13 条）</h3>
...
<div class="finding">&#9324; <b>标准化实验矩阵三件套齐装</b>（matrix_v1，Part E4&ndash;E7）：（i）<b>收敛速度</b>——proposed <b>ep95&asymp;97</b>，约 100 episode 平台化、&lt;10% 训练预算（4.4 min/run）；（ii）<b>夹逼链</b>——peak 工况 oracle 0.684 &gt; proposed 0.574 &gt; random 0.458 &gt; greedy 0.386 &gt; cache 0.313，proposed 精度较贪心 +0.19、冲突减半（0.296 vs 0.654）、deadline 0.748 vs 1.0；（iii）<b>零样本泛化</b>——UAV 数 / 负载 / SNR 三轴平滑退化、贴 oracle 上界包络、稳压 random，且零样本≈重训参考（Fig.7）。判据 5 过 4，nominal 任务成功率 0.279 vs 0.30 边缘 FAIL 如实标注。</div>
...
<tr><td>RL 实验矩阵依据</td><td><code>docs_spec/RL_Experiment_Standards_Survey.md</code>（7 篇 TWC/JSAC/IoTJ 深读，第 3 节最终矩阵）</td></tr>
<tr><td>matrix_v1 全量产物（160）</td><td><code>outputs/rl/matrix_v1/</code>：<code>matrix_summary.md</code> + <code>tables/table1_main·table2_ablation·table3_sample_efficiency</code> + <code>figures/fig1&ndash;fig7</code>（生成于 2026-07-06）；提交 <code>985554c</code>；备份 <code>matrix_v1_0707.tar.gz</code></td></tr>
...
out = "/Users/zhangqiankun/Documents/mpu/HPPO-VQA/docs_spec/UAV_VQA_SemCom_System_and_Results_v6.html"
...
# v5 原有关键数字（不删）+ v6 matrix_v1 命中项
key = ["0.6725", "1.14&times;", "&minus;0.168", "0.3027", "370.49", "2.5e-7", "primal 钝化",
       "0.574", "0.296", "0.684", "ep95&asymp;97", "0.279", "0.386", "0.313", "0.458"]
missing = [k for k in key if k not in HTML]
lenC = len(PART_C)
lenD = len(PART_D)
lenE = len(PART_E)
print(f"figs: {len(FIGS)} total (expect 16: 9 旧 + 7 新), empty={empty_figs or '无'}")
print(f"part banners: {parts} (expect 6: A-F)")
print(f"key numbers missing: {missing or '无 — 全部命中'}")
print(f"Part C chars: {lenC} | Part D chars: {lenD} | Part E chars: {lenE}")
...
assert len(FIGS) == 16, f"图数 != 16 ({len(FIGS)})"
