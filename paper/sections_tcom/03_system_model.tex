...
Before feeding the reconstructed image $\hat{I}$ into the Vision Transformer (ViT) encoder, the receiver employs a Dynamic Token Budget Allocator (\textbf{DyTBA}) to dynamically resize and tokenize the image according to a prescribed visual token budget $T_v \in \mathcal{T} = \{\mathrm{low}, \mathrm{medium}, \mathrm{high}\}$:
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Constant Symbol-Power Mode (CSPM)}: In grid-powered or macro-cell systems, the transmit power per symbol is held constant ($P_s = P_0$). The total RF transmission energy per query scales proportionally with the symbol count:
    \begin{equation}
    E_{\mathrm{tx}}^{\mathrm{CSPM}}(B) = P_0 \cdot N_s(B).
    \end{equation}
    \item \textbf{Constant Query-Energy Mode (CQEM)}: In battery-constrained edge devices (e.g., UAVs with fixed per-frame transmission energy quotas), the total energy allocated to transmit an image query is clamped at a constant budget $E_0$. Consequently, the average transmit energy per symbol adjusts inversely with the symbol count:
    \begin{equation}
    P_s(B) = \frac{E_0}{N_s(B)}.
    \end{equation}
    Under CQEM, selecting a more compact source bitrate $B_1 < B_2$ directly activates the \textbf{Physical Power Concentration (PPC)} mechanism, yielding an effective SNR boost:
