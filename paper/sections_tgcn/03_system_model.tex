...
We consider a task-oriented wireless edge VQA system where a remote edge transmitter (e.g., an aerial UAV or IoT vision sensor) captures an RGB image $I \in \mathbb{R}^{H \times W \times 3}$ and is tasked with answering a user-submitted natural language question $q$. An edge server equipped with an adapted foundational Vision--Language Model (VLM) executes multimodal reasoning to infer the ground-truth answer $A^*$. 
...
\begin{itemize}
    \item $B = 2000$ B $\implies N_{\mathrm{blocks}} = 42 \implies N_s = 21{,}420$ complex symbols.
    \item $B = 4000$ B $\implies N_{\mathrm{blocks}} = 84 \implies N_s = 42{,}840$ complex symbols.
    \item $B = 8000$ B $\implies N_{\mathrm{blocks}} = 167 \implies N_s = 85{,}170$ complex symbols.
\end{itemize}
...
A fundamental contribution of this work is the cross-layer formulation of energy consumption spanning both the wireless transmission and the edge inference phases.
...
\max_{\pi} \quad & \mathbb{E}_{(I, q, \gamma)}\left[ \mathbb{I}(\hat{A}(a) = A^*) \cdot \Pi(\gamma_{\mathrm{eff}}(B), B) \right] \\
