...
We consider a task-oriented wireless edge VQA architecture, termed EcoSem-VQA, where a remote wireless edge transmitter (e.g., an aerial UAV or IoT vision sensor) captures an RGB image $I \in \mathbb{R}^{H \times W \times 3}$ and receives a user natural language query $q$. An edge server equipped with a parameter-efficient fine-tuned Vision--Language Model (VLM) executes multimodal reasoning to infer the ground-truth answer $A^*$. 

To minimize energy consumption while guaranteeing task accuracy over an unreliable wireless link, the system dynamically co-optimizes the source transmission bitrate $B$ and the receiver visual token budget $T_v$ via a Channel-Aware Rate and Token Budgeting Network (CART-Net), denoted by $\pi$. The overall pipeline comprises three core subsystems: neural variational source compression, a digital physical layer (DigiSem-PHY) with LDPC channel coding over Rician fading, and an edge VLM equipped with a Dynamic Token Budget Allocator (DyTBA).

\subsection{Neural Source Compression and DigiSem-PHY Digital Channel Model}
...
\subsection{Receiver VLM and Dynamic Token Budget Allocator (DyTBA)}
...
Before feeding the reconstructed image $\hat{I}$ into the Vision Transformer (ViT) encoder, the receiver employs a Dynamic Token Budget Allocator (DyTBA) to dynamically resize and tokenize the image according to a prescribed visual token budget $T_v \in \mathcal{T} = \{\mathrm{low}, \mathrm{medium}, \mathrm{high}\}$:
\begin{itemize}
    \item \textit{Low Budget} ($T_v \approx 43.6$ tokens): Downsamples the input resolution to an area of $50{,}176$ pixels, capturing global semantic layout with minimal compute.
    \item \textit{Medium Budget} ($T_v \approx 110.2$ tokens): Scales the input to $100{,}352$ pixels, providing balanced spatial granularity.
    \item \textit{High Budget} ($T_v \approx 215.8$ tokens): Preserves dense resolution ($200{,}704$ pixels) for fine-grained spatial and count reasoning.
\end{itemize}
...
\subsubsection{Transmission Power and Energy Modes: CSPM vs. CQEM}
We consider two operational energy modes reflecting distinct real-world deployment constraints:
\begin{itemize}
    \item \textit{Constant Symbol-Power Mode (CSPM)}: In grid-powered or macro-cell systems, the transmit power per symbol is held constant ($P_s = P_0$). The total RF transmission energy per query scales proportionally with the symbol count:
    \begin{equation}
    E_{\mathrm{tx}}^{\mathrm{CSPM}}(B) = P_0 \cdot N_s(B).
    \end{equation}
    \item \textit{Constant Query-Energy Mode (CQEM)}: In battery-constrained edge devices (e.g., UAVs with fixed per-frame transmission energy quotas), the total energy allocated to transmit an image query is clamped at a constant budget $E_0$. Consequently, the average transmit energy per symbol adjusts inversely with the symbol count:
    \begin{equation}
    P_s(B) = \frac{E_0}{N_s(B)}.
    \end{equation}
    Under CQEM, selecting a more compact source bitrate $B_1 < B_2$ directly activates the Physical Power Concentration (PPC) mechanism, yielding an effective SNR boost:
    ...
    \end{equation}
    power advantage. Compressing from 8000 bytes ($N_s = 85{,}170$) to 2000 bytes yields a $+6.00\text{ dB}$ gain. In deep fading regimes, this $+3.01\sim+6.00\text{ dB}$ PPC boost elevates the operating SNR past the LDPC waterfall threshold, shielding the link from packet erasure.
\end{itemize}
